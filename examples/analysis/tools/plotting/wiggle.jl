...
function wiggle(pos::Vector{Float64}, sigx::Real, n::Int,
    ...
    if showplots
        ...
        plot!(p1,freqsplot/1e9,b0; label="original",c=:black,linestyle=:dash)
        plot!(p1,freqsplot/1e9,b0+Q[div(length(q)-1,2),:],label="50% quant")

        p2 = histogram(obj,label="",xlabel="Objective Value")
...
function wigglewiggle(pos::Vector{Float64},sigxs::Vector{<:Real},n::Int,
    freqsplot::Vector{Float64},bounds::Tuple{Float64,Float64}; ndiv::Int=10,nh=100)
    ...
    H = zeros(Float64,length(sigxs),nh); r = range(mn,mx,nh+1)
